    ...
    <TableHead>
      <TableRow>
        <TableCell padding='checkbox'>
          <Tooltip title='Nuevo'>
            <IconButton component={Link} to='/Servicios/Crear'>
              <AddIcon />
            </IconButton>
          </Tooltip>
        </TableCell>
      ...
      {numSelected > 0
        ? (<>
          <Tooltip title='Ver Info'>
            <IconButton component='a' href ={`/Actividad/reservada/`+idSelected} >
...
export default function TablaActividadesReservadas () {
  const [data, setData]=useState(null);
  const [error, setError] =useState('');
  const [loaded, setLoaded] =useState(false);
  useEffect(()=>{
    ActividadesReservasApi.getReservas().
    then( response=>{
